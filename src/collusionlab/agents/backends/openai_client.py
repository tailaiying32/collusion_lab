...
REASONING_MODEL_MIN_COMPLETION_TOKENS = 2048
...
class OpenAIModelClient(ModelClient):
    ...
    def _raw_call(self, messages: list[dict[str, str]], **kwargs):
        # Newer models (o1, gpt-5*, gpt-4.1*) require 'max_completion_tokens' instead of 'max_tokens'.
        ...
        is_reasoning_model = any(m in self.model_name for m in ["gpt-5", "o1-"])
        ...
        if is_reasoning_model and "max_tokens" not in kwargs:
            limit = max(limit, REASONING_MODEL_MIN_COMPLETION_TOKENS)
        if use_new_params:
